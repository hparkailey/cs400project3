...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path copyPath, Edge extendBy) {
      this.start = copyPath.start; // first vertex within path
      this.distance = copyPath.distance + extendBy.weight; // sumed weight of all edges in path
      this.dataSequence = new LinkedList<T>();
          //copyPath.dataSequence; // ordered sequence of data from vertices in
                                              // path
      ...
      this.end = extendBy.target; // last vertex within path
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    if (start == null || end == null || !this.containsVertex(start) || !this.containsVertex(end)) {
      throw new NoSuchElementException(
          "The start node is null or this graph doesn't contain starting and ending vertex specified"); 
    }

    PriorityQueue<Path> pq = new PriorityQueue<>();
    PriorityQueue<Path> completed = new PriorityQueue<>();

    ArrayList<Vertex> visited = new ArrayList<>(); 
   
    pq.add(new Path(vertices.get(start))); // add starting path to toVisit list
    visited.add(this.vertices.get(start));
    while (!pq.isEmpty()) { // while there's a node not visited
      Path currentPath = pq.poll();
     
      Vertex current = currentPath.end;
      LinkedList<Edge> edges = current.edgesLeaving;
      
      for (Edge edge : edges) { // for each edge, find the shortest path
      
        if (!this.containsEdge(current.data, edge.target.data)) {// case 0: no connecting edge can be found
          throw new NoSuchElementException("No path can be found.");
        }
        else if(edge.target.data.equals(end)) { //case 1: edge to the end node found
          Path temp = new Path(currentPath,edge);
          completed.add(temp);
        }
        else if (!visited.contains(edge.target)) { //case 2:end node not found, target node is not visited yet
          Path newPath = new Path(currentPath, edge);
          pq.add(newPath);
          visited.add(edge.target);
        } else if (visited.contains(edge.target)) { //case 3: end node not found, target node visited
          Path other = new Path(currentPath, edge);
          for(Path p: pq) {
            if(other.compareTo(p) <0 && other.end.equals(p.end)) {
              pq.add(other);
              pq.remove(p);
              }
          }
    ...
    return completed.poll(); // TODO: Implement this method in Step 7.
